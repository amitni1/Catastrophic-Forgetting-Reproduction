...
Original file is located at
    https://colab.research.google.com/drive/1lQi0YG10waGsW1MVje5W6oGSrDx8bQYe
"""
...
def compute_fisher(model, data_loader, num_samples=1024):
    ...
    - Uses labels sampled from model's own distribution (proper EWC).
    - Normalizes by number of samples (not number of batches).
    """
    fisher_matrix = {}
    ...
    for data, _ in data_loader:
        ...
        # Sample labels from model output (proper diagonal Fisher)
        sampled_labels = torch.multinomial(torch.exp(log_probs).detach(), 1).squeeze(1)
        loss = F.nll_loss(log_probs, sampled_labels)
        loss.backward()

        for name, param in model.named_parameters():
            if param.grad is not None:
                fisher_matrix[name] += param.grad.data ** 2 * batch_size
...
def test_model(model, test_loader):
    ...
    # Return as FRACTION (0-1) to match paper's Figure B y-axis
    return correct / len(test_loader.dataset)

# Parameters following Kirkpatrick et al. 2017 (EWC paper)
epochs_per_task = 20
lr = 0.001

# Lambda = 400 works well with Adam + properly normalized Fisher
# (The original paper uses ~400 for Permuted MNIST)
lamda = 400

model_sgd = Net().to(device)
model_ewc = Net().to(device)

# CRITICAL FIX: Use Adam optimizer (not SGD+momentum)
# The EWC paper uses Adam; SGD+momentum gives unstable gradients that
# overwhelm the EWC penalty, causing both models to behave the same.
...
num_tasks_plot = len(task_ids)

x_tasks = list(range(2, num_tasks_plot + 1))
ewc_avg_acc = []
sgd_avg_acc = []

for i in range(1, num_tasks_plot):
    end_epoch = (i + 1) * epochs_per_task - 1
    seen_tasks = task_ids[:i+1]

    # history values are fractions (0-1) since test_model was fixed
...
# --- Plot in style of paper Figure B ---
fig, ax = plt.subplots(figsize=(7, 4.5))

ax.plot(x_tasks, ewc_avg_acc, marker='o', color='#c44e52', label='EWC', linewidth=2)
ax.plot(x_tasks, sgd_avg_acc, marker='o', color='#4c72b0', label='SGD', linewidth=2)

# Single-task baseline: best accuracy achieved on task 0 alone (as fraction)
single_task_baseline = max(history_ewc[task_ids[0]])
ax.axhline(y=single_task_baseline, color='#333333', linestyle='--', linewidth=1.5)

# Text labels (no legend box - matches paper style)
if x_tasks:
    last_x = x_tasks[-1]
    ax.text(last_x + 0.15, ewc_avg_acc[-1], 'EWC',
            color='#c44e52', fontsize=12, va='center')
    ax.text(last_x + 0.15, sgd_avg_acc[-1], 'SGD',
            color='#4c72b0', fontsize=12, va='center')
    ax.text(last_x + 0.15, single_task_baseline + 0.003,
            'single task\nperformance', color='black', fontsize=9, va='bottom')

# Minimal axes (match paper)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
ax.tick_params(axis='both', direction='out')

# Y-axis as fraction 0-1 (MATCHES paper Figure B)
...
print(f'Single task baseline: {single_task_baseline:.3f}')
print(f'Final EWC avg: {ewc_avg_acc[-1]:.3f}')
print(f'Final SGD avg: {sgd_avg_acc[-1]:.3f}')
